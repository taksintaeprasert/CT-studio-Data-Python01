...
-- Create chat_counts table (without foreign key constraint for flexibility)
...
CREATE POLICY "Allow all operations for chat_counts" ON chat_counts
    FOR ALL
    USING (true)
    WITH CHECK (true);
...
COMMENT ON COLUMN chat_counts.staff_id IS 'Reference to staff table (soft reference)';
